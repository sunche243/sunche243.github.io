-- Preflight query (safe to run before this migration):
-- select id, created_at, pledge_option, pledge_amount
-- from public.submissions
-- where pledge_option in ('free_attending', 'free_absent')
--   and pledge_amount < 1000000
-- order by created_at;

begin;

-- Preserve every existing row: abort before any DDL if legacy custom pledges
-- below the new minimum need an explicit business decision.
do $migration$
declare
...
begin
  ...
  if v_invalid_count > 0 then
    raise exception 'Custom pledge minimum migration blocked: % existing row(s) are below 1000000', v_invalid_count
      using hint = 'Run the preflight query at the top of this migration. No existing rows were changed.';
  end if;
end;
$migration$;

-- Add and validate the stricter definition before replacing the current
-- consistency constraint, so the existing constraint remains in force.
